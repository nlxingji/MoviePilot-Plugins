...
class HotSubscribe(_PluginBase):
    ...
    # 一页多少条数据
    _movie_genre_ids: str = ""
    _tv_genre_ids: str = ""
    _anime_genre_ids: str = ""
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._movie_genre_ids = config.get("movie_genre_ids")
            self._tv_genre_ids = config.get("tv_genre_ids")
            self._anime_genre_ids = config.get("anime_genre_ids")
                ...
                if self._movie_cron:
                    try:
                        self._scheduler.add_job(func=self.__popular_subscribe,
                                                ...
                                                name="电影热门订阅",
                                                args=['电影', self._movie_page_cnt, self._movie_popular_cnt,self._movie_genre_ids])
                ...
                if self._tv_cron:
                    try:
                        self._scheduler.add_job(func=self.__popular_subscribe,
                                                ...
                                                name="电视剧热门订阅",
                                                args=['电视剧', self._tv_page_cnt, self._tv_popular_cnt,self._tv_genre_ids])
                    except Exception as err:
                ...
                if self._anime_cron:
                    try:
                        self._scheduler.add_job(func=self.__popular_subscribe,
                                                ...
                                                name="动漫热门订阅",
                                                args=['动漫', self._anime_page_cnt, self._anime_popular_cnt,self._anime_genre_ids])
                    except Exception as err:
    ...
    def __update_config(self):
        self.update_config({
            ...
            "tv_genre_ids":self._tv_genre_ids,
            "movie_genre_ids":self._movie_genre_ids,
            "anime_genre_ids":self._anime_genre_ids,
    ...
    def __popular_subscribe(self, stype, page_cnt, popular_cnt,subscribe_genre_ids: str = ""):
        """
        热门订阅
        """
        true_type = stype
        ...
        for sub in subscribes:
            ...
            _media = media
            new_media= self.downloadchain.recognize_media(mtype=_media.type, tmdbid=_media.tmdb_id,
                                             doubanid=_media.douban_id, bangumiid=_media.bangumi_id,
                                             episode_group=None)

            if new_media.category not in subscribe_genre_ids.split(","):
                logger.info(f"订阅二级分类为{subscribe_genre_ids.split(',')}")
                logger.info(f'{media.title_year} 分类：{new_media.category} 不在订阅分类{subscribe_genre_ids}中，跳过')
                continue
